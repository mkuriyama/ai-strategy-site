...
│   ├─ links.md                 関連リンク（ライブラリー／メンバーサイト／主催会社）。
│   │                           **申込導線の外側**に置く札。タブにはしない
...
> **B への導線は本文と「関連リンク」に置く**（`docs/about/index.md` の節と
> `docs/about/links.md`）。どちらも「プロジェクトについて」の配下＝**タブは3つのまま**。
>
> 外のサイト（ライブラリー・メンバーサイト・主催会社）へのリンクは
> **`{ target="_blank" rel="noopener" }` を付けて別タブで開く**。読みかけのページを
> 失わせない（B 側も同じ扱いにしてある）。
