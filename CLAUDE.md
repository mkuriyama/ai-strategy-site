...
├─ digests/                     **公開していない**（2026年9月〜）。原稿は残してあるが
│                               `hooks/export_data.py` の on_files がビルドから外している。
│                               登録前の人に過去回の詳細を大量に見せないため。叙述と図は
│                               今後 B に作る会員限定の場所へ移す。旧URLは開催履歴へ転送。
...
hooks/export_data.py            3つの役割。①`site/data/rounds.json`（回の表題・領域・開催日・
                                要約）を公開 ― B が回ページの見出しに使う。②ダイジェストを
                                ビルドから外す（公開しない）。③`DIGEST_EXPORT=1` のときだけ
                                ダイジェストを描画し `.build/digests.json`（**公開されない置き場**）
                                へ本文を書き出す ― 会員限定の場所へ運ぶ材料。
...
4. **ダイジェスト**（新しい回の分） … 下記「ダイジェストの追加手順」
   ※**公開はしない。** 原稿を置き、`extra.history` の `digests:` に対応表を足すだけ。
...
> 🔒 **ダイジェストそのものを公開しない**（2026年9月〜）。`hooks/export_data.py` が
> ビルドから外し、旧URL（`/digests/…`）は開催履歴へ転送している。公開サイトに出すのは
> 開催履歴のカード（テーマ・領域・開催日・2〜3行の要約・図1点）まで。叙述と図は、
> 今後 B に作る**会員限定の場所**（メール認証を通した先。過去の文脈からシナリオや
> ケースを組み立てる機能を置く）で使う。書き出しは `DIGEST_EXPORT=1 mkdocs build`。
> ⚠ そのビルドの `site/` にはダイジェストのページが入る。**デプロイしないこと**（CI は付けていない）。
>
